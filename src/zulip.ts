...
export const messageLoop = async (
  zulip: Zulip,
  handler: (msg: ZulipMsg) => Promise<void>
) => {
  ...
  await send(
    zulip,
    { type: "stream", to: "zulip", topic: "bots log" },
    "I started."
  );
  while (true) {
    try {
      ...
      res.events.forEach(async (event: any) => {
        ...
        if (event.type == "heartbeat") {
          ...
          if (event.message.sender_id != me.user_id) {
            event.message.command = event.message.content
              .replace(`@**${me.full_name}**`, "")
              .trim();
...
export const getStreamByName = async (
  zulip: Zulip,
...
export const reply = async (zulip: Zulip, to: ZulipMsg, text: string) =>
  await send(zulip, origToDest(to), text);
...
export const printDest = (dest: ZulipDest) =>
  dest.type == "stream" ? `\`${dest.topic}\`` : "you";
// https://chat.zulip.org/#narrow/stream/378-api-design/topic/.2Fget-subscriptions.20doc.20missmatch.3F
export const userIdFromMail = (mail: string): ZulipUserId =>
  Number(mail.split("user")[1].split("@")[0]); // bug, should already return ids
